Feature: Demo Nopcommerce's topmenu acceptance criteria
  ...
  Scenario: Verify that user can navigate to books category
    ...
    And   Click on Bookd tab on Top Menu
  ...
  Scenario: Verify that user can see 'Name: A to Z' filter is functioning as expected(Note: Products are filtered in alphbetical order)
    Given  User is on Books Category page
    When   User is on Books Category page
    And    Click on 'Sort by' filter
